...
Display-name vs domain mismatch; lookalike-domain check with **leetspeak-style character-substitution normalization** (digit-to-letter mappings like `0 -> o`, `1 -> l`, `5 -> s`, `$ -> s`) against a small brand whitelist (Microsoft, Apple, Google, common banks); freemail-impersonating-brand. Full Unicode-homoglyph normalization (Cyrillic / Greek confusables decoded via NFKC + Unicode Confusables mapping) is listed as Future Work in §12.2 — the V1 implementation catches the leetspeak class which covers a meaningful fraction of brand-impersonation phishing, and the Future Work entry names the gap honestly. Cost: $0.

### 6.3 — `urls.py` — URL reputation
Extracts URLs from text + HTML bodies, queries **VirusTotal**, **Google Safe Browsing**, and **urlscan.io** *in parallel* via `asyncio.gather`. Flags IP-as-host and known shorteners. **We never visit the URLs directly.** (HTML `<a href="X">Y</a>` link-text-vs-href mismatch detection was scoped in V1 design but not implemented — the `Signal.URL_TEXT_HREF_MISMATCH` enum is reserved for the Future Work entry in §12.2; HTML-anchor mismatch detection has real FP risk on legitimate tracking-URL marketing emails and needs a dedicated parser.)
...
- **Full Unicode-homoglyph normalization in the sender detector** — V1 ships with leetspeak character-substitution normalization (digits-as-letters like `0 -> o`); the security-research-class lookalike-domain attack uses Cyrillic / Greek / mathematical alphanumeric confusables (e.g., Cyrillic `а` U+0430 masquerading as Latin `a`). Implementing requires NFKC normalization + Unicode Confusables-mapping (`unicodedata` + a Confusables data file or the `confusables` PyPI package). The `Signal.HOMOGLYPH_IN_DOMAIN` enum value is reserved for this. Deferred because the dependency surface is small but the data-file shipping question is its own decision and the V1 leetspeak path covers a meaningful fraction of the threat without it.
- **HTML link-text-vs-href mismatch detection** — emit `Signal.URL_TEXT_HREF_MISMATCH` when the visible link text in an `<a href="X">Y</a>` tag doesn't match the actual href destination. The signal is reserved in `Signal` enum but not emitted in V1. Deferred because legitimate marketing emails routinely use tracking-redirect hrefs whose visible text is the destination brand domain, producing high false-positive rates. A production implementation would need allowlist suppression or domain-cluster comparison rather than literal string match.
