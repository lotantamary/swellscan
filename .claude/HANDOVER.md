...
**THE WOW MOMENTS MUST LAND ON BOTH CHANNELS** (added 2026-05-15). Swellscan goes through two grading lenses: (1) the README that Bar Naor reads BEFORE the interview - it's a graded artifact and the first impression of the project; (2) the 45-minute live presentation where Lotan walks through architecture, demos the Add-on against real Gmail, and answers probing questions. The product's distinguishing edges - self-defending LLM, layered detection with visible LLM short-circuit, per-sender baseline + stateless privacy story, correlation engine with named attacker playbooks, MITRE ATT&CK on every finding, lifeguard character-arc visuals, V2.S8 LLM-woven multi-signal explanations, the 12 dogfood-caught plan-drift fixes, the security-first-on-both-fronts posture - MUST be amplified deliberately in BOTH channels. The README sections should foreground these moments (not bury them in technical detail). Lotan's interview rehearsal needs to have memorable one-sentence frames for each. The next session must help Lotan prepare for the live show, not just write code: every demo screenshot needs a corresponding narrative beat ("when this happens, here's what you say"); every wow moment needs a defensible story; every choice maps to a rubric item or an Upwind published pattern. A clean README that the recruiter remembers + a live demo where she walks out citing specific signature moments = the goal.
