...
## Current State (updated 2026-05-15, Tasks 31.5 + 32 complete - cleanup + security review done)

**V1, V2, Phase 6 demos, Task 31.5 cleanup, AND Task 32 security review ALL complete and deployed.** 6 planned demos + 3 spares (demos 7, 8, 9) verified end-to-end on the deps-bumped revision. Live revision `swellscan-backend-00027-4s8` (2026-05-15). 186 tests passing (was 167). Phase 6 continues at Task 34 (README full content - **creativity emphasis**) → Task 35 (final CLAUDE.md refresh) → Task 37 (PDF cover sheet) → Task 38 (submit). Submission deadline TODAY EOD.
...
- **Live revision:** `swellscan-backend-00027-4s8` (Task 32 final - pip-audit deps bump: fastapi 0.115.6 -> 0.136.1, starlette 0.41.3 -> 1.0.0, python-dotenv 1.0.1 -> 1.2.2. All 3 CVEs cleared; live-tested all 9 demos on the new revision, all verdict labels preserved).
- **Tests:** **186 passing** (53 V1 + 83 V2 + 31 Task 31 regressions + 5 Task 31.5 urlscan-detector tests + 11 urlscan client tests + 3 send-time-anomaly range tests + 2 urlscan query-format tests). `pytest` from repo root runs all.
...
**Task 31.5 code-review skill follow-up + Task 32 commits:**

| Step | Commit | What |
|---|---|---|
| Doc refresh (pre-skill) | `2cdb075` | CLAUDE.md + design.md refreshed so the code-review skill compares current code against current docs (eliminates false-positive "design says X but code does Y" findings on already-fixed drift). design.md substantive drift fixed: detector count seven -> eight + new §6.8 bec_language, §3.1 untrusted-content tag name corrected, §3.2 SVG references reconciled with §8.3 PNG reality, §5.2 explanation max 2000, §6.3 + URL_BEHAVIORAL_FLAGGED + urlscan rationale, §6.5 PAYLOAD_FRAGMENTATION_ATTEMPT, §6.7 LLM threshold corrected from "25-75 band" to ">= 25", §7.5 timeout 30s, §9.2 multi-audience OIDC + empty-list refusal, §9.4 deploy command with OAuth client ID. |
| Code-review Tier A #1+#2 | `0bbc187` | LLM trust boundary extended to cover all three prompt tags (evidence_json, email_metadata, untrusted_content), not just the wrapped body. Plus `_sanitize_body` now applied to subject + display_name (the V2 sanitization stack was body-only; hostile Unicode Tags / EchoLeak markdown / zero-width chars in subject reached Claude un-sanitized). |
| Code-review Tier A #3 | `3ce5aef` | Send-time anomaly fix: was set-membership against observed hours (false-positive engine on any sender with a sparse history), now range check with min-3-observations gate. Demo 6's seeded hours [13-21] still fire on demo 6's 03:17 UTC arrival. |
| Code-review Tier A #4 | `ad0fc92` | CLAUDE.md deploy snippet fixed (had stale OIDC_AUDIENCE URL form + missing --max-instances=10 + missing URLSCAN_ENABLED). A re-deploy from the old snippet would have broken OIDC + cost-exhaustion ceiling. Docs-only. |
| Code-review Tier A #5 | `e2e3142` | Safe Browsing + VirusTotal exception logging redacted - dropped `error=str(exc)` because httpx exception __str__ can render the request URL into Cloud Logging, and Safe Browsing puts its API key in the URL query string (Google's v4 API contract; header form not supported). Now logs `error_type` + `status` only. |
| Code-review Tier B | `58d7d72` | design.md doc-only fixes - §6.2 "homoglyph normalization" softened to "leetspeak-style character-substitution" + Future Work entry for full Unicode confusables; §6.3 removes "link/text mismatches" claim + Future Work entry for HTML link-text-href detector. Both signal-enum values were defined but never emitted; reserved for the named Future Work items. |
| Code-review Tier C #8 | `d70a5f8` | SAFE-template `auth_passed` predicate now excludes `DMARC_FAIL`. SPF+DKIM can pass while DMARC alignment fails (the classic relay-forgery case). Without the guard, a SAFE-by-score email with DMARC_FAIL in evidence would render Variant 1 / Variant 2 body claiming the sender's "identity checks out" while DMARC_FAIL sits visible in findings. Falls through to Variant 3 ("minor things turned up") instead. |
| Code-review Tier C #9 | `f54a444` | MITRE technique IDs added to two user-visible signals: SPF_SOFTFAIL -> ["T1566.002"] matching the SPF_FAIL twin; MISSING_MESSAGE_ID -> ["T1566"] for forgery-hygiene class. FIRST_SEEN_SENDER intentionally kept empty (not an attack technique). |
| urlscan timeout fix | `38d30f6` | urlscan TIMEOUT 4.0 -> 15.0. Demo 9 verification surfaced ReadTimeout on every Cloud Run urlscan call - the 4s "cushion-tier fail-fast" theory was wrong in practice; urlscan's anonymous-tier response from Cloud Run's egress regularly exceeds 4s. Aligned with VT + SB 15s. |
| Add-on cache fix | `c6b3b87` | `addon/Code.gs` caches the first verdict per `message_id` in `CacheService.getUserCache()` with 5-min TTL. Gmail's Add-on framework re-invokes `onGmailMessageOpen` twice per user open; the first scan trains the baseline on the just-observed signals (architectural learning intent - moved under user control as Future Work), then the second scan reads the polluted baseline and returns a degraded verdict that overwrites the first card. Cache returns the first verdict on phantom re-invocation. Only demo 6 (BEC thread-hijack) showed the bug visibly. |
| HANDOVER + design refresh | `2cdb075` (earlier) | Authoritative comparison docs refreshed pre-code-review. |
| Task 32 pip-audit | `c6e074c` | pip-audit cleared 3 CVEs: python-dotenv GHSA-mf9w-mj56-hr94 (medium, set_key symlink follow), starlette GHSA-2c2j-9gv5-cj73 (medium, multipart DoS), starlette GHSA-7f5h-v6xp-fcq8 (high, FileResponse Range-header O(n^2)). fastapi 0.115.6 -> 0.136.1, starlette 0.41.3 -> 1.0.0 (major-version jump), python-dotenv 1.0.1 -> 1.2.2. pip_audit re-run after: "No known vulnerabilities found." 186 tests still passing. Live-tested all 9 demos on the bumped revision (post-deploy). |
| Task 32 security-review skill | (no commit - clean) | Final defensive sweep against both fronts (email-borne + product-targeted). Result: "No high-confidence findings beyond what is already addressed." Auth + path-serving + external clients + LLM defense + Add-on rendering + baseline storage + container hygiene + Pydantic boundaries + rate-limit keyspace + secret handling all verified clean. |
